...
public class TetraFlexScanner {

	public static final String SCANNER_NAME = "TetraFlex";
	...
	public TetraFlexScanner() {
	}
	
	public static final boolean isSelected() {
		return TetraFlexScanner.SCANNER_NAME.equals(Config.getInstance("HTS").getProperty("scanner", TetraFlexScanner.SCANNER_NAME));
	}
...
//			return new Plate[] {new Plate(8, 12, getTestTubes(0)), new Plate(8, 12, getTestTubes(1)), new Plate(8, 12, getTestTubes(2)), new Plate(8, 12, getTestTubes(3))};
//		}
		/*
		URL url = null;
		try {
			url = TetraFlexScanner.class.getResource("/resources/xtr96.reg");
			InputStream is = url.openStream();
			new File("c:/tmp").mkdirs();
			OutputStream os = new FileOutputStream("c:/tmp/xtr96.reg");
			IOUtils.redirectStream(is, os);
			is.close();
			os.close();
		...
		*/
		...
		try {
			...
			if(!directory.exists()) directory = new File("E:\\Program Files (x86)\\FluidX\\xtr-96\\");
			...
			if(!directory.exists()) directory = new File("D:\\Program Files (x86)\\FluidX\\xtr-96\\");
		...
		if(os==null) throw new IOException("Scanner not started");
		
		if(config.regEditConfig!=null) {
			InputListenerThread thread = new InputListenerThread(sock);		
			thread.start();
			os.write(("set tube = " + config.regEditConfig+"\n").getBytes());
			do {
		...
		System.out.println("TetraFlexScanner.scanPlate() "+thread.buffer);
		if(sock!=null) sock.close();
		return new Plate[] {new Plate(config.getRows(), config.getCols(), parseResults(thread.buffer))};
