...
public class PivotDlg extends JEscapeDialog {
	...
	public static List<Result> parse(Test test, String[][] table) throws Exception {
		PivotMode pivotMode;
		if(test.getAttributes(OutputType.INPUT).size()==0) {
	...
	public static List<Result> parse(Test test, String[][] table, PivotMode mode) throws Exception {
		...
		List<TestAttribute> tas = test.getAttributes(OutputType.INPUT);
		TestAttribute inputAtt = tas.size()<=0? null: tas.get(0);
		TestAttribute inputAtt2 = tas.size()<=1? null: tas.get(1);
		TestAttribute inputAtt3 = tas.size()<=2? null: tas.get(2);
		...
		int groupIndex = -1;
		//		int elnIndex = -1;
		List<Integer> missingHeaders = new ArrayList<>();
		List<String> pivotHeaders = new ArrayList<>();
		...
		if(mode!=PivotMode.ANIMAL_INPUT && tas.size()>0 && inputIndex<0) {
			if(missingHeaders.size()==0) {
				throw new Exception("The '"+tas.get(0).getName()+ "' column is missing");
			} else {
				inputIndex = missingHeaders.remove(0);
				warnings.append("The <b>"+tas.get(0).getName()+ "</b> column is missing. We assume it it the column '"+(inputIndex+1)+"'<br>");
			}
		}
	...
	private void populateTextArea() throws Exception {
		...
		if(export.size()<=0) {
			...
			for (int i = 0; i < 40; i++) {
				...
				int inputIndex = 0;
				for (TestAttribute att : test.getAttributes(OutputType.INPUT)) {
