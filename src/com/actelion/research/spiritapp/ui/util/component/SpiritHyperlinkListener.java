...
import com.actelion.research.spiritapp.ui.biosample.dialog.BiosamplePopupDlg;
				...
				protected void done() {
					if(e.getDescription().startsWith("doc:")) {
						...
						try {
							...
							System.out.println("SpiritHyperlinkListener.hyperlinkUpdate(...).new SwingWorkerExtended() {...}.done() "+id+">"+b);
							if(b!=null) {
								//								SpiritContextListener.setBiosamples(Collections.singletonList(b));
								BiosamplePopupDlg.showBiosamples(Collections.singleton(b));
							}
						} catch (Exception ex) {
							JExceptionDialog.showError(ex);
						}
