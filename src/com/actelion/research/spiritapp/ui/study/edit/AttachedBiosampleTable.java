		...
		try {
			for (AttachedBiosample b : rows) {
				if(b.getBiosample()==null) {
					Biosample biosample = new Biosample(getModel().getBiotype());
					String id = DAOBarcode.getNextId(biosample);
					biosample.setSampleId(id);
					b.setBiosample(biosample);
					b.setSampleId(id);
				} else {
					String id = DAOBarcode.getNextId(b.getBiosample());
