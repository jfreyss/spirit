...
package com.actelion.research.spiritapp.ui.study.wizard.group;
...
public class StudyGroupDlg extends JEscapeDialog {
	...
	private final ExtendTableModel<Group> groupModel = new ExtendTableModel<Group>(new Column[] {nameColumn}) {
		...
		@Override
		public List<Group> getTreeChildren(Group row) {
			return row==null? new ArrayList<>(): row.getToGroups();
		}
	};
	...
	public StudyGroupDlg(final StudyDesignDlg dlg, final Study s) {
		...
		mergeButton.addActionListener(e-> {
			...
			try {
				...
				for (int j = 1; j < selection.size(); j++) {
					...
					//Move the biosamples
					for(Biosample top: study.getParticipants(group)) {
		...
		splitButton.addActionListener(e-> {
			...
			try {
				...
				for(int subgroup=0; subgroup<group.getNSubgroups(); subgroup++) {
					...
					//Move the biosamples
					for(Biosample top: study.getParticipants(group, subgroup)) {
						for(Biosample b: top.getHierarchy(HierarchyMode.ATTACHED_SAMPLES)) {
		...
		deleteButton.addActionListener(ev-> {
			try {
				...
				for (Group group: selection) {
					if(!study.getParticipants(group).isEmpty()) throw new Exception("You cannot delete a group if there are samples attached to it");
				}
