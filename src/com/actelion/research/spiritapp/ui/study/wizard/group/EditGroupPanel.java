...
import com.actelion.research.spiritapp.ui.util.component.JColorChooserButton;
...
public class EditGroupPanel extends JPanel {
	...
	private void updateSubGroupPanel() {
		...
		try {
			...
			for (int subgroup = 0; subgroup < nSubGroups; subgroup++) {
				...
				deleteSubgroupButton.setEnabled(nSubGroups>0 && study.getParticipants(group, subgroup).size()==0);
				...
				components.add(new JInfoLabel(study.getParticipants(group, subgroup).size()+" attached samples"));
	...
	public void updateModel() throws Exception {
		...
		for (Biosample b : study.getParticipants(group)) {
