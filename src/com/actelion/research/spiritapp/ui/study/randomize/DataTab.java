...
public class DataTab extends WizardPanel {
	...
	private void populateTables(boolean comingFromCrossover, boolean populateRealTable) {
		...
		int count = 0;
		try {
			DAOResult.attachOrCreateStudyResultsToTops(study, study.getParticipantsSorted(), dlg.getPhase(), null);
		} catch(Exception e) {
	...
	public void updateView() {
		...
		for(Group g: dlg.getStudy().getGroups()) {
			...
			nAttached += dlg.getStudy().getParticipants(g).size();
			if(g.getFromGroup()!=null) nAttached += dlg.getStudy().getParticipants(g.getFromGroup()).size();
		}
