...
import com.actelion.research.spiritapp.ui.biosample.BiosampleComboBox;
...
public class SetLivingStatusDlg extends JSpiritEscapeDialog {
	...
	public SetLivingStatusDlg(final Study myStudy, List<Biosample> selectedBiosamples) {
		...
		animalList.getSelectionModel().addListSelectionListener(e-> {
			eventAnimalChanged();
		});
		...
		unsetGroupRadioButton.addActionListener(e-> {
	...
	private void eventOk() throws Exception {
		...
		for(Biosample animal: animals) {
			...
			if (replacement != null) {
				...
				Study replacementStudy = replacement.getInheritedGroup()==null? null: replacement.getInheritedGroup().getStudy();
				Group replacementGroup = replacement.getInheritedGroup();
				int replacementSubgroup = replacement.getInheritedSubGroup();
				replacement.setAttached(animal.getInheritedGroup()==null? null: animal.getInheritedGroup().getStudy(), animal.getInheritedGroup(), animal.getInheritedSubGroup());
				animal.setAttached(replacementStudy, replacementGroup, replacementSubgroup);
				...
				Container replacementContainer = replacement.getContainer();
				replacement.setContainer(animal.getContainer());
				animal.setContainer(replacementContainer);
