...
public class EditBiosamplePanel extends JPanel {
	...
	private int push = 0;
	private final JSpiritEscapeDialog dlg;
	private final EditBiosampleTable table = new EditBiosampleTable();
	private Biotype forcedBiotype;
	 ...
	 * @param dlg
	 */
	public EditBiosamplePanel(JSpiritEscapeDialog dlg) {
		this.dlg = dlg;

		setBorder(BorderFactory.createEtchedBorder());
		setLayout(new BorderLayout());

		JPanel biotypeSelectPanel = UIUtils.createHorizontalBox(new JLabel("Biotype: "), biotypeComboBox, Box.createHorizontalGlue(), setLocationButton);
		JPanel topPanel = UIUtils.createTitleBox(
				UIUtils.createVerticalBox(SpiritProperties.getInstance().isAdvancedMode()? formModeBox: null, biotypeSelectPanel));
		topPanel.setVisible(SpiritProperties.getInstance().isAdvancedMode() || forcedBiotype!=null);
		...
		} else if(biotypeComboBox.getValues().size()==1) {
			biotypeComboBox.setSelection(biotypeComboBox.getValues().iterator().next());
	...
	public void setForcedBiotype(Biotype forcedBiotype) {
		this.forcedBiotype = forcedBiotype;
		biotypeComboBox.setSelection(forcedBiotype);
		biotypeComboBox.setEnabled(forcedBiotype==null);
	}
