...
import com.actelion.research.spiritapp.spirit.ui.biosample.BiosampleSearchTree;
...
//			JButton clearButton = new JIconButton(IconType.CLEAR, "", "Clear 4 Racks");
//			clearButton.setEnabled(false);
//			clearButton.addActionListener(new ActionListener() {				
//				@Override
//				public void actionPerformed(ActionEvent e) {
//					for (RackPanel r : rackPanels) {
//						r.clear();
//					}
//					assignPositions();
//				}
//			});
//			JButton scan4Button = new JButton("4Rack-Scan");
//			scan4Button.setEnabled(false);
//			scan4Button.addActionListener(new ActionListener() {				
//				@Override
//				public void actionPerformed(ActionEvent e) {
//					assignPositions();
//				}
//			});
			...
			JPanel topPanel = UIUtils.createHorizontalBox(new JLabel("Direction: "), directionComboBox, Box.createHorizontalGlue()/*, scan4Button, clearButton*/);
			scanPanel.add(BorderLayout.NORTH, topPanel);
			
			JPanel centerPanel = new JPanel(new GridLayout(4, 1));
			for (BatchAssignRackPanel rackPanel : rackPanels) {
				centerPanel.add(rackPanel);
			}
			scanPanel.add(BorderLayout.CENTER, new JScrollPane(centerPanel));
			scanPanel.add(BorderLayout.SOUTH, UIUtils.createHorizontalBox(Box.createHorizontalGlue(), printButton, saveButton));
			
		}
					
		JPanel westPanel = new JPanel(new BorderLayout());
		westPanel.add(BorderLayout.NORTH, queryPanel);
		westPanel.add(BorderLayout.CENTER, tableScrollPane);
		
		
		//ContentPanel
		UIUtils.adaptSize(this, 1400, 900);
		JSplitPane contentPane = new JSplitPane(JSplitPane.HORIZONTAL_SPLIT, westPanel, scanPanel);
	...
	protected void assignPositions() {
		...
		if(autoCheckBox.isSelected()) {
			...
			for (int i = 0; i < biosamples.size(); i++) {
				...
				System.out.println("BatchAssignDlg.assignPositions() "+i + " (" + used+"/"+positions.size()+")" + " > "+b.getContainerId() +" "+(used<positions.size()?positions.get(used):"NA"));
//				if(b.getContainerType()==null || b.getContainerId().length()==0) continue;
				if(used>=positions.size()) break;
			...
			for (int i = 0; i < biosamples.size(); i++) {
				...
				if(rackNo==null || rackNo<0 || rackNo>=indexes.length) {
					...
					while(indexes[rackNo]<positions.size() && positions.get(indexes[rackNo]).getFirst()!=(int)rackNo) {
						indexes[rackNo]++;
					}
	...
	private void editQuery() {		
		final BiosampleSearchTree searchTree = new BiosampleSearchTree();
	...
	private void searchQuery() {
		...
		new SwingWorkerExtended("Querying", tableScrollPane, true) {
			private List<Biosample> biosamples;			
			private boolean canEdit = true;

			@Override
			protected void doInBackground() throws Exception {
				biosamples = DAOBiosample.queryBiosamples(query, Spirit.getUser());
				Collections.sort(biosamples);
				for (Biosample b : biosamples) {
					if(!SpiritRights.canEdit(b, Spirit.getUser())) {canEdit = false; break;}	
				}				
			}
	...
	private void eventSave() throws Exception {
		...
		for (Container c : containers) {
			if(c.getBiosamples().size()>0) throw new Exception("The tube "+c+" is not empty");
		}
		...
		//Set the location
		Map<Biosample, Container> oldContainers = new HashMap<Biosample, Container>();
		Map<Biosample, Location> oldLocations = new HashMap<Biosample, Location>();
		Map<Biosample, Integer> oldPos = new HashMap<Biosample, Integer>();
		try {
