...
public class BiosamplePopupDlg extends JDialog {
	...
	private Collection<Object> highlight = new ArrayList<Object>();
	...
	public void addObjects(Collection<Object> containersOrBiosamples) {
		highlight.clear();
		if(this.objects!=null) {
			this.objects.removeAll(containersOrBiosamples);
			this.objects.addAll(containersOrBiosamples);
	...
	public static BiosamplePopupDlg showBiosamples(Collection<Biosample> biosamples) {
		List<Biosample> untreated = new ArrayList<Biosample>(biosamples);
		List<Object> objects = new ArrayList<Object>();
		while(untreated.size()>0) {
			Biosample b = untreated.get(0);
			if(b.getContainer()!=null && untreated.containsAll(b.getContainer().getBiosamples())) {
				objects.add(b.getContainer());
				untreated.removeAll(b.getContainer().getBiosamples());
			} else {
				objects.add(b);
				untreated.remove(b);
