...
public class FoodWaterReport extends AbstractReport {
	...
	public FoodWaterReport() {
		super(ReportCategory.PARTICIPANTS, "Food / Water", "Food & Water consumption " + MiscUtils.convert2Html(
	...
	protected void createWorkBookNew(Workbook wb) throws Exception {
		...
		for (int i = 0; i < 2; i++) {
			...
			createHeadersWithTitle(sheet, study, i==0?"Food Consumption [g/animal/day]":"Water Consumption [ml/animal/day]");
			...
			for (Group gr : study.getGroups()) {
				if(study.getParticipants(gr).size()==0) continue;
				x = 0;
