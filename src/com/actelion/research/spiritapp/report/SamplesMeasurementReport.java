...
public class SamplesMeasurementReport extends AbstractReport {
	...
	protected void populateWorkBook() throws Exception {
		...
		loopTest: for (Test test: test2results.keySet()) {
			...
			SetHashMap<String, String> input2output = new SetHashMap<>();
			ListHashMap<String, ResultValue> key2results = new ListHashMap<>();
			...
			for(int page=0; page<nPages; page++) {
				...
				createHeadersWithTitleSubtitle(sheet , study, "Results", test.getName());
				...
				row = startRow+2;
				for(Group group: study.getGroups()) {
					ListHashMap<Integer, Integer> col2Lines = new ListHashMap<>();
					int nAnimals = 0;
					for (Biosample animal : study.getParticipants(group)) {
						if(skipEmptyAnimals && !hasData.contains(animal)) continue;
						col = 3;
