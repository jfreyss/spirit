...
public class SampleMeasurementAllGroupReport extends AbstractReport {


	private static ReportParameter REQUIRED_ONLY_PARAMETER = new ReportParameter("Show Non-Required Measurements", Boolean.FALSE);
	private static ReportParameter SHOW_OBSERVATIONS_PARAMETER = new ReportParameter("Show Observations", Boolean.TRUE);

	private Map<Group, Group> compareGroup2Groups = new HashMap<>();



	public SampleMeasurementAllGroupReport() {
		super(ReportCategory.SAMPLES,
				"Weighing (all groups)", MiscUtils.convert2Html(
						"Group1\n"
								+ "\t\t\tBW\tOrgan1\tOrgan2\n"
								+ "ANL1\t1\t33\t1.3\t0.6\n"
								+ "ANL2\t2\t31\t1.2\t0.5\n"
	...
	public JPanel getExtraParameterPanel(Study study) {
		return UIUtils.createTitleBox("Compare Groups", createCompareGroupsPanel(study, compareGroup2Groups));
	}
	...
	protected void populateWorkBook() throws Exception {
		...
		Collection<Biosample> participants = study.getParticipants();
		for(NamedSampling ns: study.getNamedSamplings()) {
			...
			for(Sampling s: ns.getAllSamplings()) {
				...
				if(s.isWeighingRequired() || !requiredOnly) {
					if(requiredOnly  || hasValue(participants, s, DAOTest.WEIGHING_TESTNAME)) {
				...
				if(s.isLengthRequired() || !requiredOnly) {
					if(requiredOnly || hasValue(participants, s, DAOTest.LENGTH_TESTNAME)) {
						lengthKeys.add(key);
						add = true;
					}
				}
				if(showObservations && (s.isCommentsRequired() || !requiredOnly)) {
					if(requiredOnly || hasValue(participants, s, DAOTest.OBSERVATION_TESTNAME)) {
						shortKeySet.add(s.getDetailsShort());
						weighingKeys.add(key);
						add = true;
		...
		for (int i = 0; i < 2+lengthsList.size(); i++) {
			...
			String sheetName = i==0? "SampleWeight": i==1?  "SampleWeight per BW": "SampleWeight per "+lengthsList.get(i-2);
			String titleName = i==0? "Sample weight": i==1? "Sample weight per BW": "Sample weight per "+lengthsList.get(i-2);
				...
				for(String s: weighingKeys) {
					...
					for(Biosample animal: study.getParticipants(group)) {
						...
						if(i==0) {
							...
							String range1 = "SampleWeight!" + convertToCell(animalRow, 5+sampleIndex);
					...
					for(String s: weighingKeys) {
						...
						for(Biosample animal: study.getParticipants(group)) {
							...
							if(v!=null && v.trim().length()>0) {
								...
								set(sheet, row, 5 + sampleIndex, animal + ": " + v.trim(), Style.S_TD_SMALL);
								row++;
	...
	private static boolean hasValue(Collection<Biosample> animals, Sampling s, String testName) {
		for(Biosample animal: animals) {
			Object v = getValueString(animal, s, testName);
